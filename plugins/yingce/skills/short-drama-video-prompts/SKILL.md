...
- 规格与 Markdown 文本照常落盘本地工作区；同时把可复制视频提示词与音乐规格用
  `canvas_create_text_node` 写成画布节点，不要只留在本地文件里。
- “投产交接”一节所指的生产=影策画布生成流程：已接受运动规格接到
  `canvas_create_generation_flow` 或 `canvas_generate_video`，首帧参考图用已有资源节点的
  `referenceNodeIds`，时长走该任务的视频参数；音乐/配音走 `canvas_generate_audio`。
  由共享 GenerationTask 与网页侧二次确认执行，不直接调用供应商接口。
