...
- 产物照常落盘本地工作区；同时把镜头表摘要与可复制关键帧提示词用 `canvas_create_text_node`
  写成画布节点，不要只留在本地文件里。
- 关键帧要真正出图时，走影策画布生成流程：`canvas_create_generation_flow` 或
  `canvas_generate_image`，参考图用已有资源节点的 `referenceNodeIds`，由共享
  GenerationTask 与网页侧二次确认执行；不直接调用供应商接口。
- 本链的 `$short-drama-produce` / `$short-drama-assets` / `$short-drama-review` 未随插件分发：
  生产=画布生成流程；资产身份=用 `canvas_get_resources` 复用已有资源节点；
  审查=把待决项清单交还创作者决定。
