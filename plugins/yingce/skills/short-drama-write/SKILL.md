...
本技能随影策 Codex 插件分发，在影策环境以独立工作区方式运行（本地文件与 python3 可用），
原有全部规则不变。已通过 yingce MCP 连接画布时，追加三条约定：

- 产物照常落盘本地工作区；同时把创作者要读的摘要（单集梗概、场景清单）用
  `canvas_create_text_node` 写成画布节点，不要只留在本地文件里。
- 本链的 `$short-drama-produce` / `$short-drama-assets` / `$short-drama-review` 未随插件分发：
  生产=影策画布生成流程（共享 GenerationTask，网页侧二次确认）；资产身份=用
  `canvas_get_resources` 复用已有资源节点；审查=把待决项清单交还创作者决定。
- 后续提示词出图/出片/出音频（含台词 TTS）时，用 `canvas_create_generation_flow` 或
  `canvas_generate_image` / `canvas_generate_video` / `canvas_generate_audio`，
  不直接调用供应商接口。
