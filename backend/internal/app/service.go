...
type Service struct {
	// TODO(merge): fork 独有字段 runtimeCapabilities / sessionCreationCoordinator /
	// sessionUploadCoordinator / mailSender 随包重构暂不可达（对应实现仍在迁移），
	// 落地 internal/app 后把字段加回此 struct 并在 newService 中初始化。
	repo *repository.Repository
	// mediaGatewayClient：MediaGateway（本地生成引擎网关）客户端，懒初始化见 mediaGateway()。
