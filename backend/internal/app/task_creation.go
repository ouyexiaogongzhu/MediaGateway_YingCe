...
func validateTaskType(taskType string) error {
	switch taskType {
	// TODO(merge): storyboard 任务类型常量原本在 internal/service/storyboard_batch.go /
	// storyboard_row_video.go（storyboard_row_video / storyboard_video_batch /
	// storyboard_music_batch / storyboard_compose）；实现迁入 internal/app 前这里用字面量。
	case "text", "agent_storyboard", "agent_storyboard_rows", "canvas_text", "canvas_image", "canvas_video", "canvas_audio",
		"storyboard_row_video", "storyboard_video_batch", "storyboard_music_batch", "storyboard_compose":
		return nil
	}
