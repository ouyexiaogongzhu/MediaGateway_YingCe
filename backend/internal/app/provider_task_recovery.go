...
func (s *Service) queryFailedVideoTask(ctx context.Context, task *model.Task, claimUserID string) (*ProviderTaskQueryResult, error) {
	...
	// 与 runVideoTask 同一套适配器选择（ensureOfficialProtocolAdapter）：
	// newapi 官方声明式适配器（OpenAI Videos / NewAPI Video Generations）由官方
	// 回退注册表提供，恢复查询的 poll/result 与创建任务协议契约一致，不随插件漂移。
